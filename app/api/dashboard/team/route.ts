...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to add sales manager" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, message: "Sales manager removed" });
  } catch (error) {
    console.error("team DELETE 错误:", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to remove sales manager" },
      { status: 500 }
    );
  }
}
